...
export const ProviderModal = ({ open, onClose, provider = {}, setProvider }) => {
  const isEditing = !!provider?._id;
  const { startCreateProvider, startUpdateProvider } = useProviderStore();

  const handleSave = async () => {
    // Validar que siempre al inicio sea status Activo (Se debe cambiar)
    ...
    >
      ...
      <Box display="flex" gap={2} mb={2} sx={{ flexDirection: 'column' }}>
        ...
        <TextField
          ...
          onChange={(e) => setProvider({ ...provider, contact_name: e.target.value })}
        />
        <TextField
          label="Numero"
          fullWidth
